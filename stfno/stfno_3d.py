...
class FNO2d_NIMRODglobal_3D(nn.Module):
    def  __init__(self, modes1, modes2,modes3, width,T_in,
        ...
        if if_model_jit_torchCompile:
            ...
            # Use factorized spectral convolution for memory compression
            for j in range(self.n_layers):
    ...
    def forward(self, x):
        ...
        for i_n_layers in range(self.n_layers):
            ...
            sum_nWidth_output_parameters_SoFar = self.nWidth_output_parameters[0]
            for i in range(1,len(self.mWidth_input_parameters)):  
                ...
                x1[i]=self.mlp_linears[i_n_layers][i](x1[i])
                x2.append(self.w_linears[i_n_layers][i](x3[i]) )        
                x[:,self.width * sum_nWidth_output_parameters_SoFar: self.width * (sum_nWidth_output_parameters_SoFar + self.nWidth_output_parameters[i]),:,:,:]= x1[i] + x2[i]
                sum_nWidth_output_parameters_SoFar += self.nWidth_output_parameters[i]
