...
# Avoid a bare "from utils import *" which may not be available on every PYTHONPATH.
# Utilities are provided via the package namespace above (stfno.utilities3).
try:
...
def initializationTrainTestParametersFile(
    ...
    # Tensor compression configuration for memory reduction
    factorization = factorization_type    # Options: 'tucker', 'cp', 'tt', 'dense'
    rank = factorization_rank              # Compression ratio: 0.05=95% reduction, 0.1=90% reduction, 0.5=50% reduction
    
    # Select device and allow fallback from CUDA OOM to CPU
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # Diagnostics: print device/GPU information to confirm execution path
    try:
        cuda_visible = os.environ.get('CUDA_VISIBLE_DEVICES', '')
        print(f"Device selection: {device} | torch.cuda.is_available()={torch.cuda.is_available()} | CUDA_VISIBLE_DEVICES='{cuda_visible}')")
        if device.type == 'cuda':
            try:
            ...
            try:
                ...
                print(f"GPU[{idx}]: name='{props.name}', capability={props.major}.{props.minor}, multiprocessors={props.multi_processor_count}")
            except Exception as e:
                print(f"GPU properties unavailable: {e}")
            try:
                print(f"CUDA runtime: {getattr(torch.version, 'cuda', 'unknown')}, cuDNN: {torch.backends.cudnn.version()}")
            except Exception:
                pass
    except Exception:
        pass
    ...
    if if_IncludeSteadyState:
        if if_model_Nimrod_STFNO_global:
            model = FNO2d_global            (modes, modes,modes, width,(T_in+T_in_steadystate),sum_vector_a_elements_i_iter,T_out,sum_vector_u_elements_i_iter,number_of_layers)
        else:
            model = FNO2d_glob_orig         (modes, modes, width,(T_in+T_in_steadystate),sum_vector_a_elements_i_iter)
    else:
        if if_model_Nimrod_STFNO_global:
            if if_3D:
                ...
                # mWidth_input_parameters can be an int or a list/tuple; compute total channels
                try:
                    if isinstance(mWidth_input_parameters, (list, tuple)):
                        channels = T_in * sum(int(x) for x in mWidth_input_parameters)
                    else:
                        channels = T_in * int(mWidth_input_parameters)
                except Exception:
                    channels = T_in * 1
                input_size = (int(batch_size), int(S_r), int(S_theta), int(S_n_phi), int(channels))
                # Torchinfo can allocate a CUDA sample tensor; guard to avoid OOM
                try:
                    summary(model, input_size=input_size, device=str(device))
                except RuntimeError as e:
                    if 'CUDA' in str(e).upper() or 'OOM' in str(e).upper():
                        print('Skipping model summary due to device/memory constraint:', str(e).split('\n')[0])
                    else:
                        raise
            else:
            ...
            if if_3D:
                ...
                model = FNO2d_glob_orig         (modes, modes, width,(T_in)  ,sum_vector_a_elements_i_iter)

    # Move to device with CUDA OOM fallback
    try:
        model = model.to(device)
    except RuntimeError as e:
        if 'CUDA' in str(e).upper():
            print('CUDA OOM or not available; falling back to CPU for the model move')
            device = torch.device('cpu')
            model = model.to(device)
        else:
            raise
    # Confirm final model device
    try:
    ...
    if output_base_dir is not None and not os.path.exists(output_base_dir):
    ...
    if output_base_dir is not None:
        strn_epochs_dump_path_file1 = os.path.join(output_base_dir, 'epochs_output_'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    else:
        strn_epochs_dump_path_file1 = os.path.join('data', 'epochs_output_'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    for k_fieldlist_parm_eq_vector_train_global_lst_i_j, fieldlist_parm_eq_vector_train_global_lst_i_j_k in enumerate(fieldlist_parm_eq_vector_train_global_lst_i_j[0]):
    ...
    if output_base_dir is not None:
        strn_epochs_dump_path_file2 = os.path.join(output_base_dir, 'epochs_diags_'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    else:
        strn_epochs_dump_path_file2 = os.path.join('data', 'epochs_diags_'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    for k_fieldlist_parm_eq_vector_train_global_lst_i_j, fieldlist_parm_eq_vector_train_global_lst_i_j_k in enumerate(fieldlist_parm_eq_vector_train_global_lst_i_j[0]):
    ...
    file2.close()
    if output_base_dir is not None:
        strn_epochs_dump_path_file3 = os.path.join(output_base_dir, 'epochs_diagsMaxNormRel_'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    else:
        strn_epochs_dump_path_file3 = os.path.join('data', 'epochs_diagsMaxNormRel_'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    for k_fieldlist_parm_eq_vector_train_global_lst_i_j, fieldlist_parm_eq_vector_train_global_lst_i_j_k in enumerate(fieldlist_parm_eq_vector_train_global_lst_i_j[0]):
    ...
    file3.close()
    if output_base_dir is not None:
        strn_epochs_dump_path_file4 = os.path.join(output_base_dir, 'epochs_diagsMaxNormRelList_'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    else:
        strn_epochs_dump_path_file4 = os.path.join('data', 'epochs_diagsMaxNormRelList_'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    for k_fieldlist_parm_eq_vector_train_global_lst_i_j, fieldlist_parm_eq_vector_train_global_lst_i_j_k in enumerate(fieldlist_parm_eq_vector_train_global_lst_i_j[0]):
    ...
    file4.close()
    if output_base_dir is not None:
        strn_epochs_dump_path_file5 = os.path.join(output_base_dir, 'epochs_diagsTime_ntestZerosSize1only_'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    else:
        strn_epochs_dump_path_file5 = os.path.join('data', 'epochs_diagsTime_ntestZerosSize1only_'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    for k_fieldlist_parm_eq_vector_train_global_lst_i_j, fieldlist_parm_eq_vector_train_global_lst_i_j_k in enumerate(fieldlist_parm_eq_vector_train_global_lst_i_j[0]):
    ...
    file5.close()
    if output_base_dir is not None:
        strn_epochs_dump_path_file6 = os.path.join(output_base_dir, 'epochs_diagsTime_jit_torchcompile'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    else:
        strn_epochs_dump_path_file6 = os.path.join('data', 'epochs_diagsTime_jit_torchcompile'+str(i_fieldlist_parm_eq_vector_train_global_lst)+'__')
    for k_fieldlist_parm_eq_vector_train_global_lst_i_j, fieldlist_parm_eq_vector_train_global_lst_i_j_k in enumerate(fieldlist_parm_eq_vector_train_global_lst_i_j[0]):
