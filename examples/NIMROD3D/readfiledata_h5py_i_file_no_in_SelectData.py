    ...
    for i_fieldlist_parm_lst, fieldlist_parm_lst_i in enumerate(fieldlist_parm_lst):
            ...
            for i_fieldlist_parm_eq_selected, fieldlist_parm_eq_selected in enumerate(range(fieldlist_parm_eq_range)):
                for i_fieldlist_parm_vector_i, fieldlist_parm_vector_i in  enumerate(range(fieldlist_parm_vector_lst[i_fieldlist_parm_lst])):
                    ...
                    h5file.close()
                    if if_3D:
                        # Subsample data if needed to fit target dimensions
                        data_tensor = torch.from_numpy(data_read)
                        if data_tensor.shape != (data_read_global_hdf5file.shape[-3], data_read_global_hdf5file.shape[-2], data_read_global_hdf5file.shape[-1]):
                            # Subsample evenly across all dimensions
                            step_r = data_tensor.shape[1] // S_r if data_tensor.shape[1] > S_r else 1
                            step_theta = data_tensor.shape[2] // S_theta if data_tensor.shape[2] > S_theta else 1
                            step_phi = data_tensor.shape[3] // S_n_phi if data_tensor.shape[3] > S_n_phi else 1
                            data_tensor = data_tensor[:, ::step_r, ::step_theta, ::step_phi][:, :S_r, :S_theta, :S_n_phi]
                        data_read_global_hdf5file[:,i_fieldlist_parm_lst,i_fieldlist_parm_eq_selected,i_fieldlist_parm_vector_i,:,:,:] = data_tensor.to(data_read_global)
                    else: 
