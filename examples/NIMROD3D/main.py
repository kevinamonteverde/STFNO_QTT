...
# Add project directories to Python path (robust, relative-to-this-file)
# ROOT points to the STFNO_QTT package root; EXAMPLES_DIR to the examples/NIMROD3D folder.
ROOT = pathlib.Path(__file__).resolve().parents[1].parents[0]
EXAMPLES_DIR = pathlib.Path(__file__).resolve().parent
# Insert examples dir first so local example scripts are importable, then package root
...
print(f"========================================")
print(f"Factorization Type: {factorization_type}")
print(f"Factorization Rank: {factorization_rank}")
...
# Missing variables that were causing NameError
T_in = 1   # Input time steps
T_out = 1   # Output time steps
step = 1    # Time step increment
T_out_sub_time_consecutiveIterator_factor = 1  # Time consecutive iterator factor

print(f" T_in = {T_in}")
print(f" T_out = {T_out}")
print(f" step = {step}")
print(f" T_out_sub_time_consecutiveIterator_factor = {T_out_sub_time_consecutiveIterator_factor}")
...
if_SVD_analysis = False
if_plot_singular_value_decay = False
...
if if_HyperDiffusivity_case:
    ...
    ntest  = 66 # Reduced for testing (was 66)
else: 
...
S_r = S 
S_theta = S
S_n_phi = 32 #32 #64 #S  # Match the actual data size
i_nphi_plot2DContour = 0
S_r_hdf5file = S_r # >= S_r
S_theta_hdf5file = S_theta  # >= S_theta
S_phi_hdf5file = 32 #32 #64 # >= S_phi  # Match the actual data size
...
S_r_hdf5file = S_r # >= S_r
S_theta_hdf5file = S_theta  # >= S_theta
S_phi_hdf5file = 32 #32 #nx_phi #64 # >= S_phi  # Match the actual data size
nlvls =50
log_param= False
